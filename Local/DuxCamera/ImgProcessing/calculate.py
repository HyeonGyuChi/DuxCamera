...
def clac_MeanBright(img, q_point_info, degree=1) : 
    print('\n┌━━━ Call func, [mean_bright] ━━━-┐')
    ...
    plt.figure(figsize=(15,15)) # 25개 roi
    ...
    for i in range(h): # 5
        for j in range(w) : # 5
            ...
            # 평균밝기 연산
            roi = img[right_y : left_y+1, left_x : right_x+1] # 밝기 참조영역
            ...
            cv2.rectangle(img_copy, (left_x, left_y), (right_x, right_y), (1000, 1000, 1000), 2)
            cv2.circle(img_copy, (x, y), 1, (1000, 1000, 1000), -1)
            cv2.putText(img_copy, str(q_idx), (left_x - 30, left_y - 30), 
            cv2.FONT_HERSHEY_SIMPLEX, 1, (1000, 1000, 1000), 1)
            
            
            tube_mean_bright[i,j] = mean_bright
            q_idx += 1
            
    print('REF BLOB SHAPE : ', roi.shape)
    plt.show();
    plt.figure(figsize=(15,15)); plt.imshow(img_copy,  cmap='gray', vmin=0, vmax=255*4), plt.title('TOTAL ROI'), plt.show();
    
    
    print('└━━━ Return func, [mean_bright] ━━━┘\n')
